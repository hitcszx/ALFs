...
log_file = open('./results/'+args.dataset+'_'+args.noise_type+'_'+str(epochs)+'_'+str(args.noise_rate)+'.log', 'w')

if args.dataset == 'MNIST':
    criterions, names = get_mnist_exp_criterions_and_names(num_classes)
elif args.dataset == 'CIFAR10':
    criterions, names = get_cifar10_exp_criterions_and_names(num_classes, train_loader)
elif args.dataset == 'CIFAR100':
    criterions, names = get_cifar100_exp_criterions_and_names(num_classes, train_loader)
print(names)

for criterion, label in zip(criterions, names):
    log(log_file, label)
